...
async function loadProviderRequests(providerId) {  
  ...
  // Fetch the provider's username
  const providerDoc = await getDoc(doc(db, "users", providerId));
  const providerUsername = providerDoc.exists() ? providerDoc.data().username || "Unknown Provider" : "Unknown Provider";

  querySnapshot.forEach((doc) => {  
    const data = doc.data();  
    requestsDiv.innerHTML += `  
      <p><b>Service:</b> ${data.serviceName} | <b>Assigned To:</b> ${providerUsername} | <b>Status:</b> ${data.status}</p>  
    `;  
  });  
}  
...
async function loadAdminDashboard(dashboard) {  
  ...
  for (const doc of querySnapshot.docs) {  
    const data = doc.data();  
    // Fetch usernames for requestedBy and assignedTo
    let requesterUsername = "Unknown User";
    let providerUsername = "Unassigned";
    
    if (data.requestedBy) {  
      const requesterDoc = await getDoc(doc(db, "users", data.requestedBy));  
      requesterUsername = requesterDoc.exists() ? requesterDoc.data().username || "Unknown User" : "Unknown User";  
    }  
    if (data.assignedTo) {  
      const providerDoc = await getDoc(doc(db, "users", data.assignedTo));  
      providerUsername = providerDoc.exists() ? providerDoc.data().username || "Unknown Provider" : "Unknown Provider";  
    }  

    adminRequestsDiv.innerHTML += `  
      <p><b>Service:</b> ${data.serviceName} | <b>Requested By:</b> ${requesterUsername} |   
      <b>Assigned To:</b> ${providerUsername} | <b>Status:</b> ${data.status}</p>  
    `;  
  }  
}  
